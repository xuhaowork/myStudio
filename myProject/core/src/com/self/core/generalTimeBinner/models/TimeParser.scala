...
class TimeParser(val timeColInfo: TimeColInfo) extends Serializable {
  def parse(time: String): BinningTime = timeColInfo match {
  ...
  def parse(time: Long): BinningTime = timeColInfo match {
  ...
  def parse(time: Timestamp): BinningTime = timeColInfo match {
    case _: StringTypeTimeColInfo =>
  ...
  def parse(time: BinningTime): BinningTime = timeColInfo match {
    case _: StringTypeTimeColInfo =>
      throw new SparkException("数据为BinningTime类型，但您输入的时间列信息为string类型")
    case _: LongTypeTimeColInfo =>
      throw new SparkException("数据为BinningTime类型，但您输入的时间列信息为long类型")
    case _: TimestampTypeTimeColInfo =>
      throw new SparkException("数据为BinningTime类型，但您输入的时间列信息为timestamp类型")
    case _: BinningTypeTimeColInfo =>
      time
  }


  def parse(time: Any): BinningTime = time match {
    case t: String => parse(t)
    case l: Long => parse(l)
    case ts: Timestamp => parse(ts)
    case bt: BinningTime => parse(bt)
    case other => throw new SparkException(s"类型只支持string, long, Timestamp, BinningTime类型, " +
      s"您输入的数据${other}数据类型为${other.getClass.getSimpleName}")
  }
