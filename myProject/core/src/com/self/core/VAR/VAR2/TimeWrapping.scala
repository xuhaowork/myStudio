package com.self.core.VAR.VAR2
...
object TimeWrapping extends BaseMain{
  override def run(): Unit = {
    ...
    var rawDataDF = z1.rdd(tableName).asInstanceOf[org.apache.spark.sql.DataFrame]

    /** 2)获取id属性信息 */
    val idColInfoObj = pJsonParser.getAsJsonObject("idColInfo")
    val hasIdCol = idColInfoObj.get("value").getAsString // 是否有id列
    ...
    /** 3)获取时间列信息 */
    val timeCol = pJsonParser.getAsJsonArray("timeCol").get(0).getAsJsonObject
      .get("name").getAsString

    /** 3)获取时间列的字段属性 */
    val fieldFormatObj = pJsonParser.getAsJsonObject("fieldFormat")
    val fieldFormat = fieldFormatObj.get("value").getAsString

    /** 4)时间序列区间选择和观测窗宽选择 */
    val intervalInfo = pJsonParser.getAsJsonObject("fieldFormat")
    val intervalType = intervalInfo.get("intervalType").getAsString // 按最大最小时间平滑 --sliding
    ...
    rawDataDF = fieldFormat match {
      case "StringType" =>
        if (col_type != StringType) {
          ...
          val timeFormat = fieldFormatObj.get("timeFormat").getAsString
    ...
    val (startTimeStamp: Long, endTimeStamp: Long) = util.Try(intervalType match {
      case "byHand" => {
        ...
        val startTime = intervalInfo.get("startTime").getAsString
        val endTime = intervalInfo.get("endTime").getAsString
      ...
      case "sliding" => {
        ...
        val (startT, endT) = minMax.rdd.map(r => (r.getAs[Long](0), r.getAs[Long](1))).collect().head
        val slidingUnit = intervalInfo.get("endTime").getAsString
        slidingUnit match {
          case "year" => {
            val start_dt = new DateTime(startT)
            val end_dt = new DateTime(endT)
            (start_dt.yearOfCentury().roundFloorCopy().getMillis,
              end_dt.yearOfCentury().roundFloorCopy().getMillis)
          }
          case "season" => {
            val start_dt = new DateTime(startT)
            val end_dt = new DateTime(endT)
            val minus_month_start = (start_dt.getMonthOfYear - 1) % 3
            val minus_month_end = (end_dt.getMonthOfYear - 1) % 3
            (start_dt.minusMonths(minus_month_start).monthOfYear().roundFloorCopy().getMillis,
              end_dt.minusMonths(minus_month_end).monthOfYear().roundFloorCopy().getMillis)
          }
          case "month" => {
            val start_dt = new DateTime(startT)
            val end_dt = new DateTime(endT)
            (start_dt.monthOfYear().roundFloorCopy().getMillis,
              end_dt.monthOfYear().roundFloorCopy().getMillis)
          }
          case "weekday" => {
            val start_dt = new DateTime(startT)
            val end_dt = new DateTime(endT)
            (start_dt.weekOfWeekyear().roundFloorCopy().getMillis,
              end_dt.weekOfWeekyear().roundFloorCopy().getMillis)
          }
          case s: String => {
            val slingLength = s match {
            ...
            ((startT + 28800000) / slingLength * slingLength - 28800000,
              (endT + 28800000) / slingLength * slingLength - 28800000)
          }
        }
      }
    }) getOrElse (throw new Exception("类型不对"))

    val frequencyObj: JsonObject = pJsonParser.getAsJsonObject("frequencyObj")

    def getZeroValue(frequencyFormat: String, startTimeStamp: Long, endTimeStamp: Long, frequencyObj: JsonObject)
    : Array[Long] = frequencyFormat match {
      case "fixedLength" => {
        val frequencyLength = frequencyObj.get("frequencyLength").getAsString
        val unit = frequencyObj.get("unit").getAsString
        val frequency: Int = {
    ...
    if(hasIdCol == "false"){
      ...
      val idColInfo = idColInfoObj.get("idColName").getAsJsonArray.get(0).getAsJsonObject
      idColName = idColInfo.get("name").getAsString
      idColType = idColInfo.get("datatype").getAsString
    }
    ...
    val zeroValue: Map[Long, (Long, Array[Double])] = getZeroValue(frequencyFormat,
      startTimeStamp, endTimeStamp, frequencyObj).map(roundTime =>
      (roundTime, (Long.MaxValue, Array.fill(K)(Double.NaN)))).toMap
