...
private[generalTimeBinning] class BinningTime(
  ...
  def binning(timeBinnerInfo: TimeBinnerInfo): this.type = {
    ...
    val residue = new RelativeMeta(residueResult, None, adaptFormat4Residue, DateTimeZone.UTC)
...
private[generalTimeBinning] class RelativeMeta(
  ...
  def this(value: Long, timeFormat: String) = this(value, None, timeFormat, DateTimeZone.UTC)

  override def createNew(value: Long, rightBound: Option[Long]): TimeMeta =
    new RelativeMeta(value, rightBound, timeFormat, DateTimeZone.UTC)

  override def toString: String = {
    val dt = new DateTime(value).withZone(timeZone)
    println(dt, dt.getZone)
    var str = new DateTime(value).withZone(timeZone).toString(timeFormat)
    if (rightBound.isDefined)
      str += "," + new DateTime(rightBound.get).withZone(timeZone).toString(timeFormat)
    "[" + str + "]"
  }
