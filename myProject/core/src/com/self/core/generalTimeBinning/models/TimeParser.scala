...
class TimeParser(val timeColInfo: TimeColInfo) extends Serializable {
  def parse(time: String): TimeMeta = {
    timeColInfo match {
      case st: StringTypeTimeColInfo =>
        val timeFormat = st.timeFormat
        if (timeFormat.toLowerCase contains "yyyy") {
          try {
            new AbsTimeMeta(new SimpleDateFormat(timeFormat).parse(time).getTime)
          } catch {
            case e: Exception =>
          ...
          new RelativeMeta(new SimpleDateFormat(timeFormat).parse(time).getTime)
        }
  ...
  def parse(time: Long): TimeMeta = {
    timeColInfo match {
      ...
      case lt: LongTypeTimeColInfo =>
        ...
        try {
          new AbsTimeMeta(if (unit == "millisecond") time else time * 1000)
        } catch {
          case e: Exception =>
  ...
  def parse(time: Timestamp): TimeMeta = {
    timeColInfo match {
      case _: StringTypeTimeColInfo =>
