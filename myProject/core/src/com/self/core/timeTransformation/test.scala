...
import com.self.core.timeTransformation.models.{StringTimeFormatMatcher, TimeFormatParser, UDFCreator}
import org.apache.spark.sql.types.{LongType, StringType, TimestampType}
import org.apache.spark.sql.{DataFrame, NullableFunctions, UserDefinedFunction}
...
object test extends myAPP {
  ...
  def test(): Unit = {
    ...
    Tools.columnExists(timeFieldName, rawDataFrame, true)
  ...
  def zzjzScripts(): Unit = {
    ...
    /** 分别对每条记录进行模式匹配 */
    val rawDataFrame1 = z1.rdd("timeDF").asInstanceOf[DataFrame]
    val stringTimeFormatMatcher = new StringTimeFormatMatcher()
    rawDataFrame1.show()
    times.foreach {
      time => println(stringTimeFormatMatcher.parse(time))
    }

    val parse = NullableFunctions.udf(
      (time: String) => {
        val dt = stringTimeFormatMatcher.parse(time)
        new Timestamp(if (dt != null) dt.getTime else 0L)
      }
    )
    import org.apache.spark.sql.functions.col
    val newDataFrame = rawDataFrame1.select(col("time"), parse(col("time")).as("parseTime"))
    newDataFrame.show
